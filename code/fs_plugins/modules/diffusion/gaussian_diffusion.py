...
from fs_plugins.modules.diffusion.nn import mean_flat
from fs_plugins.modules.diffusion.losses import normal_kl, discretized_gaussian_log_likelihood, discretized_text_log_likelihood
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        # print(x.shape)
        # model_output = model(x, self._scale_timesteps(t), **model_kwargs)
        model_output = model.model.decoder.forward_x_t(
            normalize=False,
            prev_output_tokens=model_kwargs['prev_decoder_out'][0],
            encoder_out=model_kwargs['encoder_out'],
            x_t=x,
            t=t
        )
    ...
    def training_losses_e2e(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.model_arch == 'conv-unet':
            ...
            x_start_mean = x_start_mean.view(x_start_mean.size(0), seqlen, seqlen, x_styouart_mean.size(-1)).permute(0, 3,
                                                                                                                  1, 2)
        elif self.model_arch == '1d-unet':
