...
class CTCDecoderBase:
    ...
    """
    def __init__(self, dictionary: Dictionary, decoder_parameters: Dict) -> None:
    ...
    def ctc_post_processing(self, sentence_index: List[int]) -> List[int]:
        ...
        sentence_index = list(filter((self.dictionary.blank_index).__ne__, sentence_index))
        sentence_index = list(filter((self.dictionary.pad_index).__ne__, sentence_index))
...
class CTCScopeSearchLengthControlDecoder(CTCDecoderBase):
    ...
    def __init__(self, dictionary, decoder_parameters):
        ...
        self.blank_index = dictionary.blank_index
    ...
    def scope_search_row_inference(self, column, reshaping_index, only_blank_prob, only_non_blank_prob,
        ...
        transition_index = torch.zeros(self.beam_size, device=self.device, dtype=torch.int16) - 1
    ...
    def ctc_scope_search_length_control_initialization(self, logits):
        ...
        # This assertion results from our int16 declaration of transition tracking tensor
        assert self.ctc_sequence_length <= 2 ** 15, "The sequence length cannot exceed 128"
        ...
        self.transition_tracker_table \
            = torch.zeros(transition_tracker_dimensions, dtype=torch.int16, device=self.device) - 1
    ...
    def determine_whether_length_control_needed(self, logits, source_length):
        ...
        if self.use_length_ratio:
            ...
            self.sample_desired_length = max(int(np.floor(self.desired_length * source_length)), 2) + 2
        else:
