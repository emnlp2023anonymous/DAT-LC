...
class MultilingualIterativeRefinementGeneratorWithBacktranslation(object):
    def __init__(
        self,
        tgt_dict,
        models=None,
        ...
        # self.tgt_lang_tok = tgt_lang_tok
        self.all_tgt_lang_tok = all_tgt_lang_tok

    @property
    def symbols_to_strip_from_output(self):
        return {self.eos, *self.all_tgt_lang_tok}
    ...
    @torch.no_grad()
    def generate(self, models, sample, prefix_tokens=None, constraints=None, postprocess=False):
        ...
        encoder_out = model.forward_encoder(src_tokens, src_lengths, src_lang_id=src_lang_id)

        # decoder_langtok = tgt_lang_id if getattr(model.args, "langtok_input", False) else None

        prev_decoder_out = model.initialize_output_tokens(encoder_out, src_tokens, None)
        ...
        for step in range(self.max_iter + 1):
            ...
            decoder_out = model.forward_decoder(
                prev_decoder_out, encoder_out, tgt_lang_id=tgt_lang_id, **decoder_options
            )
        ...
        if postprocess:
            for hypo in finalized:
                hypo[0]['tokens'] = hypo[0]['tokens'][hypo[0]['tokens'] not in self.symbols_to_strip_from_output]
        return finalized
