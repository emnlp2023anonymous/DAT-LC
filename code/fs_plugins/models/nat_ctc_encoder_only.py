...
@register_model("nat_ctc_encoder_only")
class NATransformerModel(FairseqNATModel):
    def __init__(self, args, encoder, decoder):
        ...
        try:
            ...
            print("CTC is not installed !!!")
        
        self.build_length_control_decoder()
    ...
    def build_length_control_decoder(self):
        from fs_plugins.others.ctc_length_control_decoding import CTCScopeSearchLengthControlDecoder
        ...
        lc_beam_size = getattr(self.args, 'lc_beam_size', None)
        margin_criteria = getattr(self.args, 'margin_criteria', None)

        decoding_setting = {
            "force_length": False,
            "use_length_ratio": specified_length_ratio,
            "k": 6,
            "beam_size": lc_beam_size,
            "marg_criteria": margin_criteria,
            "desired_length": specified_length_fixed,
            "truncate_summary": False,
            "scope": 2
        }
        self.lc_decoding = CTCScopeSearchLengthControlDecoder(self.tgt_dict, decoding_setting)
    ...
    def forward_decoder(self, decoder_out, encoder_out, decoding_format=None, **kwargs):
        ...
        if specified_length_fixed is not None or specified_length_ratio is not None:
            def _pad_lc_output(tokens):
                _toks = tokens
                max_len = max([len(x) for x in _toks])
                _tokens = [_a_toks + [self.tgt_dict.pad_index] * (max_len - len(_a_toks)) for _a_toks in _toks]
                hyp = torch.tensor(_tokens).to(output_tokens)
                return hyp
            
            lc_tokens = self.lc_decoding.decode(output_logits, encoder_out["src_lengths"][0].view(-1))
            output_tokens = _pad_lc_output(lc_tokens)
            return decoder_out._replace(
                    output_tokens=output_tokens,
                    output_scores=torch.full(output_tokens.size(), 1.0, device=output_logits.device),
                    attn=None,
                    history=history,
                )
            ...
            if self.plain_ctc:
                ...
                if hasattr(self.tgt_dict, "blank_index"):
                    extra_symbols_to_ignore.append(self.tgt_dict.blank_index)
                def _ctc_postprocess(tokens):
                    hyp = tokens
                    # if cfg.task.plain_ctc:
                    _toks = hyp.int().tolist()
                    _toks = [[v for i, v in enumerate(_a_toks) if i == 0 or v != _a_toks[i - 1]] for _a_toks in _toks]
                    _toks = [[v for v in _a_toks if v not in extra_symbols_to_ignore] for _a_toks in _toks]
                    max_len = max([len(x) for x in _toks])
                    _tokens = [_a_toks + [self.tgt_dict.pad_index] * (max_len - len(_a_toks)) for _a_toks in _toks]
                    hyp = hyp.new_tensor(_tokens)
                    return hyp
                
                # output_scores = decoder_out.output_scores
                _scores, _tokens = output_logits.max(-1)
                output_tokens = _ctc_postprocess(_tokens)
                ...
                return decoder_out._replace(
                    ...
                    output_scores=torch.full(top_beam_tokens.size(), 1.0, device=output_logits.device),
                    attn=None,
                    history=history,
                )
...
@register_model_architecture(
    "nat_ctc_encoder_only", "nat_ctc_encoder_only"
)
